...
class VasProvisioning
{
    ...
    private function parseXML($str)
    {
        ...
        $xml_detail = substr($str, strpos($str, "<RPLY"));
    ...
    public function vasRegisterPackage($msisdn, $service, $transaction_id, $channel_type, $promotion = 0, $promotion_node = '', $trial = 0)
    {
        ...
        if ($service->serviceProvider) {
            $this->app = $service->serviceProvider->vasgate_cp_id;
            $this->svc = $service->serviceProvider->provisioning_app;
            $this->username = $service->serviceProvider->vivas_gw_username;
        }else{
    ...
    {
        if ($action == 'subscribe') {
            $user = Subscriber::findByMsisdn($msisdn, $service->service_provider_id, true);
            ...
            $user = Subscriber::findByMsisdn($msisdn, $service->service_provider_id);
            $transaction = SubscriberTransaction::findOne(['id' => $transaction_id]);
